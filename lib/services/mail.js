...
module.exports = class MailService extends Service {
    ...
    static async sendRemovedFilmMail(user, film) {
        const transporter = nodemailer.createTransport({
            host: process.env.MAIL_HOST,
            port: process.env.MAIL_PASSWORD,
            auth: {
        ...
        let message = {
            from: process.env.MAIL_SITE_EMAIL,
            to: user.email,
            subject: 'Removed film',
            text: `Hi ${user.userName} ! ${film.title} as been removed from API.`
        };
    ...
    static async sendMailModificationFilmMail(user, film) {
        const transporter = nodemailer.createTransport({
            host: process.env.MAIL_HOST,
            port: process.env.MAIL_PASSWORD,
            auth: {
        ...
        let message = {
            from: process.env.MAIL_SITE_EMAIL,
            to: user.email,
            subject: 'Updated a film',
            text: `Hi ${user.userName} ! ${film.title} as been updated from API !`
        };
