...
module.exports = [
    ...
    },
    {
        ...
        options: {
            ...
            validate: {
                payload: Joi.object( {
                    field : Joi.string().required().min(3).example('userName').description('The field in db that need to be changed'),
                    newValue : Joi.any().required().example('1, newmail@domain.tld, ...').description('The new value'),
                })
            }
        },
        handler: async (request, h) => {
